...
// 获取所有订单信息
let { data: res } = await useFetch(`http://110.42.190.78:8888/api/orderall?id=${user.value.no}`, { server: false });
watchEffect(() => {
    if (res.value) {
...
const orderId = async (id) => {
    tagger.value = false
    // 获取对应订单的信息
    const { data } = await useFetch(`http://110.42.190.78:8888/api/orderlist?id=${id}`, { server: false })
    watchEffect(() => {
        if (data.value) {
            if (data.value.order) {
