...
const { user } = useCount()
let { data } = await useFetch('http://110.42.190.78:8888/api/user/getUser', { server: false,credentials:'include' })
watchEffect(() => {
    if (data.value) {
...
const goodss = ref([])
const { data: res } = await useFetch(`http://110.42.190.78:8888/api/cartList?userNo=${user.value.no}`, {
    server: false,
    credentials:'include'
})
watchEffect(() => {
...
const dele = async (row) => {
    // 向数据库发送请求删除对应的商品
    const { data } = await useFetch(`http://110.42.190.78:8888/api/delete?id=${row.goodsNo}`,{
        credentials:'include'
    })
...
const numberChange = async (row) => {
    const { data } = await useFetch(`http://110.42.190.78:8888/api/updata?id=${row.goodsNo}&count=${row.count}&userNo=${user.value.no}`,{
        credentials:'include'
    })
    if (data.value.code !== 0) {
