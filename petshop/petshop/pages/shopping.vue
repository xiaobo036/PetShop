...
const types = ref("");
let { data } = await useFetch("http://110.42.190.78:8888/api/user/getUser", {
  server: false,
  credentials:'include'
});
...
watch(route, async (routes) => {
  const { data: res1 } = await useFetch(
    `http://110.42.190.78:8888/api/test/goods?id=${routes.query.id}`
  );
  watchEffect(() => {
    if (res1.value) {
...
const { data: res2 } = await useFetch(
  `http://110.42.190.78:8888/api/test/goods?id=${route.query.id}`
);
watchEffect(() => {
  if (res2.value) {
...
const handleChange = () => {};
// 立即购买
// { goodsNo: 3, img: 'http://110.42.190.78:8888/img/4.png', title: '维斯康 VITSCAN宠物保健品海藻颗粒', taste: '颜色:红色', price: 98, count: 1, userNo: userNo.value }
const createCart = async (goods) => {
  if (user.value.no) {
    ...
    const { data } = await useFetch("http://110.42.190.78:8888/api/addcart", {
      method: "post",
      body: obj,
    });
    isf.value = true;
