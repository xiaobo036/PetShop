...
let { data: res } = await useFetch("http://110.42.190.78:8888/api/user/getUser", {
  server: false,
  credentials:'include'
});
...
const { data: res1 } = await useFetch(
  `http://110.42.190.78:8888/api/cartList?userNo=${user.value.no}`,
  {
    server: false,
    credentials:'include'
  }
...
var { data } = await useFetch(`http://110.42.190.78:8888/api/city`);
  ...
  Form.value.validate(async (valid) => {
    if (valid) {
      ...
      const { data } = await useFetch(
        "http://110.42.190.78:8888/api/user/address",
        {
          method: "post",
          body: {
...
const pay = () => {
  Form.value.validate(async (valid) => {
    if (valid) {
      ...
      obj.name = ruleForm.name;
      obj.phone = ruleForm.phone;
      const { data } = await useFetch("http://110.42.190.78:8888/api/order", {
        method: "post",
        body: {
          address:
            value1.value + value2.value + value3.value + ruleForm.address,
      ...
      const { data: res } = await useFetch(
        `http://110.42.190.78:8888/api/deletecart?id=${user.value.no}`
      );
      watchEffect(() => {
        if (data.value) {
          if (data.value.code === 0) {
  ...
  Form.value.validate(async (valid) => {
    if (valid) {
      ...
      const { data: res2 } = await useFetch(
        `http://110.42.190.78:8888/api/test1?orderNo=${orderNo}`
      );
      let address = ref();
      watchEffect(() => {
        if (res2.value) {
